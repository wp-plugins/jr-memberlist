...
if (!function_exists("jr_memberlist_refresh")) {
function jr_memberlist_refresh() {
update_option("jr_submitted_memberlist", "0");
}
}
...
update_option("jr_submitted_memberlist", "1");
wp_schedule_single_event(time()+172800, 'jr_memberlist_refresh'); 
} else {
...
update_option("jr_memberlist_link_personal", $content);
}
}
...
$linkper=utf8_decode(get_option('jr_memberlist_link_personal'));

if (get_option("jr_memberlist_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_memberlist_links_choice'));
...
update_option("jr_memberlist_links_choice", $new);
update_option("jr_memberlist_link_newcheck", "444");
}

if (get_option("jr_submitted_memberlist")=="0") {
$pname="jr_memberlist";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_memberlist", "1");
...
wp_schedule_single_event(time()+172800, 'jr_memberlist_refresh'); 
} else if (get_option("jr_submitted_memberlist")=="") {
$pname="jr_memberlist";
$url=get_bloginfo('url');
$current=get_option("jr_memberlist_links_choice");
...
wp_schedule_single_event(time()+172800, 'jr_memberlist_refresh'); 
  ...
  $pshow = "<p style='font-size:x-small'>Memberlist Plugin created by ".$linkper." - ".stripslashes(get_option('jr_memberlist_links_choice'))."</p>";
  echo $pshow;
}
